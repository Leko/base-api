...
class Orders extends Client
{
	 ...
	 *   @param XXXXX start_ordered 注文日時はじめ yyyy-mm-dd または yyyy-mm-dd hh:mm:ss (任意)
	 *   @param XXXXX end_ordered 注文日時おわり yyyy-mm-dd または yyyy-mm-dd hh:mm:ss (任意)
	 ...
	 * @return array 連想配列。ドキュメントのサンプルレスポンスを参照
	 */
	public function :unique_key(array $params = array()) {
		return $this->request('get', '/1/orders/detail/:unique_key', $params);
	}
	 ...
	 * 
	 * @param array $params 指定可能なオプションは以下を参照
	 *   @param XXXXX order_item_id 購入商品ID (必須)
	 *   @param XXXXX status dispatched か cancelled のいずれか (必須)
	 *   @param XXXXX add_comment 発送メールに添付する一言メッセージ
	 * @return array 連想配列。ドキュメントのサンプルレスポンスを参照
	 */
